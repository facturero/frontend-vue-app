...
|---|---|---|---|
| 1 | ⚠️ El secuencial de número de factura se lee/incrementa **sin lock atómico** pese a que el doc promete `SELECT...FOR UPDATE`; no hay constraint único en `invoices.number` | billing-service | Dos emisiones concurrentes sobre el mismo punto de emisión pueden generar el mismo número de factura |
| 2 | ⚠️ Agregar una línea con `productId` inexistente **no falla** — crea la línea sin snapshot ni impuestos, silenciosamente | billing-service | Factura "válida" con datos incompletos |
...
| 5 | `update-customer` no revalida unicidad de identificación al cambiarla | customer-service | Se puede dejar a un cliente con la misma identificación que otro de la misma organización |
| 6 | `LastAdminRemovalError` está definido pero **nunca se lanza** — solo se protege al `ownerId` explícito, no "el último admin" | auth-service | Con 2+ admins sin ninguno marcado como owner, se puede desactivar a todos |
...
| 9 | Revocar un permiso a un usuario logueado **no lo bloquea de inmediato** — el JWT vigente sigue sirviendo hasta refresh/expirar (900s) | auth-service | Ventana de permisos obsoletos tras un cambio de rol |
...
| `known-bugs.spec.ts` | Los 15 hallazgos originales del resumen ejecutivo (#1-15), con 4 `test.skip()` documentados |

Resumen ejecutivo: **24 hallazgos totales (1-21 + #22 documentado en §3 + #23 en §7 +
#24 en el gateway)**, 7 corregidos (#16 en frontend; #18, #19, #20, #21, #24 en
backend — #16 se corrigió en la verificación del 2026-09-06, lo que implicó actualizar
el test que documentaba el fallo silencioso, ver `customers.spec.ts`; #24 se encontró y
corrigió escribiendo el caso de "reusar token de reset" de §1.4, ver
`employees.spec.ts`). Suite completa: `npx playwright test` — 70 tests (65 pasan, 5
