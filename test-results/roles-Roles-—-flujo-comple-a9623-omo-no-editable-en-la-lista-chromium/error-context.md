...
- Name: roles.spec.ts >> Roles — flujo completo >> rol de sistema se muestra como no editable en la lista
- Location: e2e\specs\roles.spec.ts:31:3
...
Locator: getByText('sistema').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('sistema').first()
